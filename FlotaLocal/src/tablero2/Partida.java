...
public class Partida {
    ...
    public JSONObject pruebaCasilla(int f, int c) {
        ...
        int valor = mar[f][c];
        if (valor == AGUA || valor == TOCADO || valor == HUNDIDO) {
            resultado.put("resultado", valor);
            return resultado;
        }

        Barco barco = barcos.get(valor);
        barco.tocaBarco();
        ...
        if (barco.estaHundido()) {
            ...
            mar[f][c] = TOCADO;
            resultado.put("resultado", TOCADO);
        }
        return resultado;
    }
